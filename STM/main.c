...
int main(void) {
    ...
    //  motorTaskHandle = osThreadNew(StartMotorTask, NULL,
    ...
    //  bulleyesTaskHandle = osThreadNew(StartBulleyesTask, NULL,
    //  &bulleyesTask_attributes);
    //
    //	/* creation of encoderRightTas */
    //	encoderRightTasHandle = osThreadNew(StartEncoderRightTask, NULL,
    //&encoderRightTas_attributes);
    //
    //  /* creation of encoderLeftTask */
    //  encoderLeftTaskHandle = osThreadNew(StartEncoderLeftTask, NULL,
    //  &encoderLeftTask_attributes);
    ...
    /* creation of bulleyesTask */
    bulleyesTaskHandle =
        osThreadNew(StartBulleyesTask, NULL, &bulleyesTask_attributes);

    /* creation of encoderRightTas */
    encoderRightTasHandle =
        osThreadNew(StartEncoderRightTask, NULL, &encoderRightTas_attributes);

    /* creation of encoderLeftTask */
    encoderLeftTaskHandle =
        osThreadNew(StartEncoderLeftTask, NULL, &encoderLeftTask_attributes);
...
/*
  hardcoded distance 2150
  forward == 1 or 0, 1 if forward, 0 if reverse
 */
void moveCarSlideRight(int forward) {
    int sign;
    if (forward) {
...
void task2Algo() {
    /*
      while ultrasonic dont detect wall -> move foward
      scan -> get direction
      move left/right (route around and face forward)
      while ultrasonic dont detect wall -> move foward
      scan -> get direction
      check distance in front -> if smaller thaqn threshold -> reverse first
      move left/right (route around and go back)
      TODO: find out how to track forward distance while moving
     */
    // TODO: global var to store vertical distance

    // TODO: find out how to use ultrasonic
    while (ultrasonic() > 30) {
        osDelay(10);
        moveCarStraight(1000);  // 10cm?
        vertDist = vertDist + 1000;
    }

    char dir = scanAndReceive();  // returns l, r
    if (strcmp(dir, 'l')) {
        left();
    ...
    dir = ' ';
    // not enough space -> move back
    if (ultrasonic() < 30) {
        osDelay(10);
        moveCarStraight(-2000);
        vertDist = vertDist - 2000;
    } else {  // move foward until obstcale
        while (ultrasonic() > 30) {
            osDelay(10);
            moveCarStraight(1000);  // 10cm?
            vertDist = vertDist + 1000;
    ...
    // TODO: find out how to send scan command (comms)
}
...
void left() {
     ...
     */
    moveCarLeft(90);
    vertDist = vertDist + 2000;  // assuming im following constants.py from algo
    moveCarRight(90);
    vertDist = vertDist + 3000;  // cos facing west
    moveCarRight(90);
    vertDist = vertDist + 2000;  // cos facing north
    moveCarLeft(90);
    vertDist = vertDist + 3000;  // cos facing east
}
...
void secondLeft() {
    /*
      move backwards -> l -> r -> r -> move foward 50cm -> r -> move forward
      distance traveled - 10cm r -> l
     */
    moveCarLeft(90);
    moveCarRight(90);
    moveCarRight(90);
    moveCarStraight(5000);  // assumme 5k = 50cm
    moveCarRight(90);
    for (int i = 0; i < /* global vert movement - 1000*/; i = i + 1000) {
...
void secondRight() {
    /*
      move backwards -> r -> l -> l -> move foward 50cm -> l -> move forward
      distance traveled - 10cm l -> r
    */
    moveCarRight(90);
    moveCarLeft(90);
    moveCarLeft(90);
    ...
    if (right) {          // rightMotor
        if (error > 0) {  // go forward
            HAL_GPIO_WritePin(GPIOA, BIN1_Pin,
                              GPIO_PIN_SET);  // set direction of rotation for
                                              // wheel B- forward
            HAL_GPIO_WritePin(GPIOA, BIN2_Pin, GPIO_PIN_RESET);
        } else {  // go backward
            HAL_GPIO_WritePin(GPIOA, BIN1_Pin,
                              GPIO_PIN_RESET);  // set direction of rotation for
                                                // wheel B - reverse
...
void StartMotorTask(void *argument) {
    ...
    HAL_GPIO_WritePin(
        GPIOA, BIN1_Pin,
        GPIO_PIN_SET);  // set direction of rotation for wheel B- forward
    HAL_GPIO_WritePin(GPIOA, BIN2_Pin, GPIO_PIN_RESET);
    HAL_GPIO_WritePin(
        GPIOA, AIN1_Pin,
        GPIO_PIN_SET);  // set direction of rotation for wheel A - forward
    HAL_GPIO_WritePin(GPIOA, AIN2_Pin, GPIO_PIN_RESET);
    osDelay(1000);
    ...
    for (;;) {
        ...
        if (pwmVal_servo == 148) {
            ...
            pwmVal_R = PID_Control(rightTarget - rightEncoderVal, 1) +
                       200;  // right is master
            ...
            if (rightTarget - rightEncoderVal > 0) {
                // go forward
                HAL_GPIO_WritePin(GPIOA, AIN1_Pin,
                                  GPIO_PIN_SET);  // set direction of rotation
                                                  // for wheel A- forward
                HAL_GPIO_WritePin(GPIOA, AIN2_Pin, GPIO_PIN_RESET);
            } else {
                // go backward
                HAL_GPIO_WritePin(GPIOA, AIN1_Pin,
                                  GPIO_PIN_RESET);  // set direction of rotation
                                                    // for wheel A - reverse
                HAL_GPIO_WritePin(GPIOA, AIN2_Pin, GPIO_PIN_SET);
            }
        }
...
void StartOledTask(void *argument) {
    ...
    /* Infinite loop */
    for (;;) {
        sprintf(clear, "L:%d | R:%d     ", (int)(leftTarget - leftEncoderVal),
                (int)(rightTarget - rightEncoderVal));
...
void StartGyroTask(void *argument) {
    ...
    for (;;) {
        ...
        if (HAL_GetTick() - tick >= 1000L) {
            ...
            // for gyro drift removal, calibration needs to be added to angular
            // speed
            total_angle += (double)(angular_speed + 2.7) *
...
void StartBulleyesTask(void *argument) {
    ...
    for (;;) {
        while (i < 1) {
            //		  while(rightTarget>rightEncoderVal &&
            // leftTarget>leftEncoderVal){ 			  osDelay(100);
            //		  }
            //		  osDelay(1300);
